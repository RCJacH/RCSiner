...
def  main():
  ...
  installer = "\installer\RCSiner Installer.exe"
   
  if demo:
    installer = "\installer\RCSiner Demo Installer.exe"
   
  FILES_TO_ZIP = [
    projectpath + installer,
    projectpath + "\installer\changelog.txt",
    projectpath + "\installer\known-issues.txt",
    projectpath + "\manual\RCSiner manual.pdf" 
  ]
  ...
  ZIPNAME = "RCSiner-v" + FULLVERSIONSTR + "-win.zip"
  
  if demo:
    ZIPNAME = "RCSiner-v" + FULLVERSIONSTR + "-win-demo.zip"
